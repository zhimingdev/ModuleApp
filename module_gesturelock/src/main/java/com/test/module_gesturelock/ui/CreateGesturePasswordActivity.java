...
public class CreateGesturePasswordActivity extends BaseMvpActivity implements
...
//        flag = getIntent().getStringExtra("flag");
    ...
    private void initStatusHeight() {
        int statusHeight = getStatusBarHeights(this);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ViewGroup.MarginLayoutParams mlp = (ViewGroup.MarginLayoutParams) vFull.getLayoutParams();
            mlp.height += statusHeight/2;
            vFull.setLayoutParams(mlp);
        }
    }
    ...
    public void setCommonTopHeight() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            //int height = ZlqUtils.getStatusBarHeight(this);
    ...
    private void saveChosenPatternAndFinish() {
        SharedPreferences sharedPreferences = getSharedPreferences("key", Context.MODE_PRIVATE); //私有数据
        SharedPreferences.Editor editor = sharedPreferences.edit();
        String s1 = LockPatternUtils.patternToString(mChosenPattern);
        String key = sharedPreferences.getString("1", "");
        System.out.println("加密手势是"+s1 +"-----------"+key);
        if (key.equals(s1)) {
            ...
            showToast("手势密码设置成功");
            String s = LockPatternUtils.patternToString(mChosenPattern);
...
//        if ("islocked".equals(flag)) {
//            Intent upgesIntent = new Intent(this,
//                    UnlockGesturePasswordActivity.class);
//            upgesIntent.putExtra("flag", "islocked");
//            startActivity(upgesIntent);
//            finish();
//        } else if ("regist".equals(flag)) {
//            Intent upgesIntent = new Intent(this,
//                    UnlockGesturePasswordActivity.class);
//            upgesIntent.putExtra("flag", "regist");
//            startActivity(upgesIntent);
//            finish();
//        } else {
//            finish();
